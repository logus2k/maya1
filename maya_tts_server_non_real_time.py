...
VOICE_DESCRIPTION_MAP = {
    # American English Female - High quality
    'af_heart': {
        'description': 'Realistic female voice in their 30s with american accent. Warm timbre, conversational pacing, friendly tone at medium intensity.',
        'temperature': 0.4,
        'lang': 'en'
    },
    'af_bella': {
        'description': 'Realistic female voice in their 20s with american accent. Bright timbre, energetic pacing, upbeat tone at high intensity.',
        'temperature': 0.4,
        'lang': 'en'
    },
    'af_nicole': {
        'description': 'Realistic female voice in their 30s with american accent. Clear timbre, professional pacing, neutral tone at medium intensity.',
        'temperature': 0.4,
        'lang': 'en'
    },
    'af_sarah': {
        'description': 'Realistic female voice in their 40s with american accent. Rich timbre, calm pacing, mature tone at medium intensity.',
        'temperature': 0.4,
        'lang': 'en'
    },
    ...
    'am_michael': {
        'description': 'Realistic male voice in their 30s with american accent. Normal pitch, warm timbre, conversational pacing, neutral tone at medium intensity.',
        'temperature': 0.4,
        'lang': 'en'
    },
    'am_adam': {
        'description': 'Realistic male voice in their 40s with american accent. Deep pitch, authoritative timbre, steady pacing, confident tone at medium intensity.',
        'temperature': 0.4,
        'lang': 'en'
    },
    ...
    'bf_emma': {
        'description': 'Realistic female voice in their 30s with british accent. Refined timbre, measured pacing, polite tone at medium intensity.',
        'temperature': 0.4,
        'lang': 'en'
    },
    'bf_alice': {
        'description': 'Realistic female voice in their 20s with british accent. Light timbre, quick pacing, cheerful tone at medium intensity.',
        'temperature': 0.4,
        'lang': 'en'
    },
    ...
    'bm_george': {
        'description': 'Realistic male voice in their 40s with british accent. Deep timbre, formal pacing, professional tone at medium intensity.',
        'temperature': 0.4,
        'lang': 'en'
    },
    'bm_lewis': {
        'description': 'Realistic male voice in their 30s with british accent. Clear timbre, conversational pacing, friendly tone at medium intensity.',
        'temperature': 0.4,
        'lang': 'en'
    },
}
...
class SNACDecoder:
    ...
    def decode_to_bytes(self, snac_tokens: List[int]) -> Optional[bytes]:
        """Decode SNAC tokens to audio bytes (int16 PCM)."""
        audio = self.decode(snac_tokens)
        if audio is None:
            return None
...
class Maya1VoiceModel:
    ...
    def build_prompt(self, description: str, text: str) -> str:
        """
        Build Maya-1-Voice prompt using chat template.
        
        Format: <description="..."> text
        
        Args:
            description: Voice description
            text: Text to synthesize (optionally with <emotion> tags)
...
class Maya1VoiceStreamingPipeline:
    ...
    async def generate_speech_stream(
        ...
        """
        Generate speech audio with clean streaming (no artifacts).
        
        Collects all tokens, then decodes once to ensure artifact-free audio.
        """
        ...
        if token_buffer:
            complete_frames = len(token_buffer) // 7
            frames_decoded = complete_frames
            tokens_to_use = complete_frames * 7
            
            if tokens_to_use < len(token_buffer):
                logger.debug(f"⚠️  Truncating {len(token_buffer) - tokens_to_use} incomplete tokens")
                token_buffer = token_buffer[:tokens_to_use]
            
            logger.info(f"🎵 Decoding {tokens_to_use} tokens ({complete_frames} frames)")
            
            # Decode all at once for clean audio
            audio_bytes = self.snac_decoder.decode_to_bytes(token_buffer)
            
            if audio_bytes:
                logger.info(f"✅ Decoded {len(audio_bytes)} bytes of clean audio")
                yield audio_bytes
            else:
                logger.error(f"❌ Decoding failed")
        else:
            logger.warning(f"⚠️  No SNAC tokens generated")
        
        logger.info(f"✅ Streaming complete: {total_tokens} tokens → {frames_decoded} frames")
...
async def generate_speech(sid, data):
    ...
    try:
        ...
        temperature = voice_config.get('temperature', tts_settings.get('temperature', 0.4))
